<script setup lang="ts">
  ...
  const props = defineProps({
    tags: {
      type: Array as PropType<UserTag[]>,
      required: true,
    },
    isLoading: {
      type: Boolean,
  ...
  const getGroupInfo = (letter: string) => {
    ...
    const totalCount = tags.reduce((sum, tag) => sum + (tag.file_count || 0), 0)
    return {
      count: tags.length,
...
<template>
  <div class="tag-cloud-container">
    <div
      v-if="isLoading"
      v-loading="{ loading: true, text: $t('tagManage.loading'), type: 'default' }"
      class="loading-state"
    ></div>
    ...
    <div v-else-if="tags.length === 0" class="empty-state">
      <div class="empty-content animate-slide-up">
        ...
        <h3 class="empty-title">{{ $t('tagManage.empty.title') }}</h3>
        <p class="empty-desc">{{ $t('tagManage.empty.description') }}</p>
      </div>
    </div>
        ...
        <div v-for="letter in availableLetters" :key="letter" :data-letter-group="letter" class="letter-group">
          <div class="letter-group-header">
            ...
            <div class="letter-stats">
              <span class="stat-item">
                ...
                {{ $t('tagManage.tagCloud.groupCount', { count: getGroupInfo(letter).count }) }}
              </span>
              <span class="stat-divider">·</span>
              <span class="stat-item">
                <i class="fas fa-chart-line mr-1"></i>
                {{ $t('tagManage.tagCloud.groupUsage', { usage: getGroupInfo(letter).totalUsage }) }}
              </span>
            </div>
          </div>
          ...
          <div class="tags-grid">
            <div
              ...
              class="tag-item"
              :class="[
                { selected: selectedTagIds.includes(tag.id) || tempSelectedTagIds.includes(tag.id) },
            ...
            >
              ...
              <div v-if="highlightedTagId === tag.id && !isBatchMode" class="tag-actions" :class="getActionPosition(tag.id)">
                <button class="action-btn edit-btn" :title="$t('tagManage.actions.edit')" @click.stop="$emit('edit', tag)">
                  <i class="fas fa-edit" />
                </button>
                <button class="action-btn delete-btn" :title="$t('tagManage.actions.delete')" @click.stop="$emit('delete', tag)">
          ...
          <div
            ...
            :title="
              availableLetters.includes(letter)
                ? $t('tagManage.tagCloud.letterNav', { letter, count: getGroupInfo(letter).count })
                : $t('tagManage.tagCloud.letterEmpty', { letter })
            "
