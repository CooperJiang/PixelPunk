    ...
    try {
      const uploadPromises = files.map(async (file) => {
        ...
        if (res.code === 200 && res.data) {
          ...
          throw new Error(res.message || $t('admin.construction.terms.uploadFailed'))
        }
      })

      const urls = await Promise.all(uploadPromises)
      callback(urls)
      toast.success($t('admin.construction.terms.uploadSuccess').replace('{count}', urls.length.toString()))
    } catch (error: any) {
      toast.error($t('admin.construction.terms.uploadError') + error.message)
      callback([])
    }
  }
    ...
    return [
      {
        ...
        value: localSettings.terms_of_service_content,
        type: 'text',
        group: 'legal',
        description: $t('admin.construction.terms.title'),
      },
    ]
  }
    ...
    <div class="settings-section">
      <div class="section-header">
        <div class="section-title">
          ...
          <h3>{{ $t('admin.construction.terms.title') }}</h3>
        </div>
        <p class="section-description">{{ $t('admin.construction.terms.description') }}</p>
      </div>

      <div class="editor-container">
        <MdEditor
          v-model="localSettings.terms_of_service_content"
          language="zh-CN"
          :placeholder="$t('admin.construction.terms.placeholder')"
      ...
      <div class="editor-hint">
        ...
        <span>{{ $t('admin.construction.terms.hint') }}</span>
      </div>
    </div>
  </div>
</template>
